          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {result && (
              <button
                type="button"
                onClick={() => navigate('/')}
                className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-700 shadow-sm"
              >
                Go to Dashboard
              </button>
            )}
        ...
        <Card header="Pipeline result" subtitle="Output from the ML prediction and correction engine.">
          ...
          {result && (
            <div className="space-y-4">
              ...
              {result.terminal_logs && (
                <div className="space-y-1 text-xs mt-4">
                  <p className="font-semibold text-slate-500 uppercase tracking-wide">Pipeline Console Output</p>
                  <div className="rounded-xl bg-slate-950 p-3 font-mono text-[10px] text-slate-200 overflow-x-auto max-h-48 border border-slate-800 shadow-inner">
                    <pre className="whitespace-pre-wrap leading-relaxed select-text">{result.terminal_logs}</pre>
                  </div>
                </div>
              )}
            </div>
